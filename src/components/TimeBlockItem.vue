...
async function onUp() {
  ...
  const edge = dragEdge
  dragEdge = null
  if (localStart.value !== props.entry.startMinutes || localEnd.value !== props.entry.endMinutes) {
    // Only count as edit (not click) if position changed
    emit('updated', props.entry.id, localStart.value, localEnd.value)
    await dayStore.updateEntry(props.entry.id, props.entry.projectId, localStart.value, localEnd.value, props.entry.note)
  } else if (edge === 'move') {
...
<template>
  ...
  >
    ...
    <div class="block-body" @mousedown.stop="startDrag('move', $event)" @click.stop="onClick">
      ...
      <span v-if="height > 38" class="duration">{{ minutesToTime(localStart) }} – {{ minutesToTime(localEnd) }} · {{ formatDuration(durationMin) }}</span>
...
.block-body:active { cursor: grabbing; }
